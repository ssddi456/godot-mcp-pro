...
用于 AI 驱动 Godot 游戏开发的高级 MCP（Model Context Protocol）服务器。将 Claude 等 AI 助手直接连接到你的 Godot 编辑器，在 Full 模式下最多提供 **172 个强大工具**。

## 架构

```
AI Assistant ←—stdio/MCP—→ Node.js Client ←—WebSocket:6520—→ Node.js 或 Go Daemon ←—WebSocket:6505-6509—→ Godot Editor Plugin
```
...
- `server-go/` — 与 Node.js stdio/CLI 客户端兼容的 Go daemon
...
构建 Node.js MCP stdio/CLI 客户端和 Node daemon：
...
Node.js 和 Go daemon 都在 `6520` 端口提供相同的本地 WebSocket JSON-RPC API，因此 Node.js MCP stdio/CLI 客户端可以连接任一 daemon。

### 3. 启动 daemon

启动一个 daemon，并在使用 MCP 客户端时保持运行：
...
允许所有 MCP 工具和所有 Bash 命令。显式拒绝破坏性 shell 命令（`rm -rf`、`git push --force`、`git reset --hard` 等）以及上述相同的破坏性 MCP 工具。

> **注意**：如果你已有 `settings.local.json`，请手动合并 `permissions` 部分，而不是覆盖。

### 6. Lite 模式（可选）

如果你的 MCP 客户端有工具数量限制（例如 Windsurf：100，Cursor：~40），可以使用 Lite 模式，注册 81 个核心工具而不是 172 个：
...
| 分类 | Godot MCP Pro | GDAI MCP | tomyud1 | Dokujaa | Coding-Solo | ee0pdt | bradypp |
|----------|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
...
| Other | 0 | 0 | 9 | 5 | 5 | 2 | 1 |
| **总计** | **172** | ~30 | **32** | **39** | **13** | **19** | **10** |

### 功能矩阵

| 功能 | Godot MCP Pro | GDAI MCP | tomyud1 | Dokujaa | Coding-Solo |
|---------|:---:|:---:|:---:|:---:|:---:|
